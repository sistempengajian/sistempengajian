...
import MaterialRollingFormModal from './MaterialRollingFormModal';
import { deleteMaterialRolling } from '@/app/(protected)/jadwal/rolling-materi/actions';
...
interface MaterialRollingListViewProps {
  initialRollings: any[];
  generations: GenerationOption[];
  canManage: boolean;
}

export default function MaterialRollingListView({
  initialRollings = [],
  generations = [],
  canManage = true,
}: MaterialRollingListViewProps) {
  ...
  const filteredRollings = rollings.filter((r) => {
    if (searchQuery.trim()) {
      ...
      const matchMat = r.queues?.some((queue: any) =>
        queue.items?.some((it: any) =>
    ...
    if (selectedGenFilter !== 'ALL' && r.targetGenerationId !== selectedGenFilter) {
      return false;
    }
  ...
  const handleDelete = (id: string, name: string) => {
    if (!confirm(`Apakah Anda yakin ingin menghapus rolling materi "${name}"?`)) {
      return;
    }

    startTransition(async () => {
      const res = await deleteMaterialRolling(id);
      if (res.error) {
        alert(res.error);
      } else {
  ...
  const handleSuccess = () => {
    // Reload halaman secara halus agar data terbaru terambil
    window.location.reload();
  };

  // Helper badge warna wilayah
  ...
  // Helper badge tipe rolling
  const getRollingTypeBadge = (type: RollingIntervalType) => {
    switch (type) {
      case 'PER_PENGAJIAN':
        return (
          <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-teal-50 text-teal-800 border border-teal-200/80 flex items-center gap-1">
  ...
  return (
    <div className="space-y-5">
      {/* Navigation Breadcrumb / Kembali ke Jadwal */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white/80 backdrop-blur-md rounded-2xl border border-slate-200/80 p-4 shadow-2xs">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-bold text-base sm:text-lg text-slate-900 tracking-tight leading-tight">
              Kelola Rolling Materi
            </h2>
            <p className="text-xs text-slate-500 mt-0.5">
              Tahap 1: Master antrean materi bergulir per pengajian, mingguan, atau bulanan
            </p>
          </div>
        </div>
        ...
        {canManage && (
          <button
            ...
            className="inline-flex items-center justify-center gap-2 px-4 py-2 bg-teal-600 hover:bg-teal-500 text-white rounded-xl text-xs font-bold transition-all active:scale-95 shadow-2xs cursor-pointer shrink-0"
          >
            <Plus className="w-4 h-4" />
            <span>Buat Rolling Materi Baru</span>
          </button>
        )}
      </div>
      ...
      <div className="bg-white/80 backdrop-blur-md rounded-2xl border border-slate-200/80 p-3 sm:p-4 shadow-2xs flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Cari nama rolling materi atau judul materi di dalamnya..."
        ...
        {generations.length > 0 && (
          <select
            ...
            className="px-3.5 py-2 text-xs bg-slate-50/80 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-teal-500/20 text-slate-800 font-medium shrink-0"
          >
      ...
      <div className="space-y-3.5">
          ...
          return (
            ...
            >
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="space-y-1.5 flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-1.5">
                    ...
                    {rolling.targetGeneration && (
                      <span className="px-2.5 py-0.5 rounded-full text-[10px] font-medium bg-slate-100 text-slate-600 border border-slate-200/60">
                        {rolling.targetGeneration.name}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col justify-start text-left items-start gap-x-4 gap-y-1 text-xs text-slate-500">
                    <span>
                      Kapasitas:{' '}
                      <strong className="text-slate-800 font-semibold">
                        {rolling.itemsPerSession} Materi / Pengajian
                      </strong>
                    </span>
                    <span>
                      Panjang Antrean:{' '}
              ...
              <div className="pt-2 border-t border-slate-100">
                ...
                >
                  <span>
                    {isExpanded ? 'Sembunyikan Urutan Materi' : 'Lihat Susunan Urutan Antrean'}
                  </span>
                  {isExpanded ? (
                ...
                {isExpanded && (
                  <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5 animate-in fade-in">
                    {rolling.queues?.map((q: any) => (
                      <div
                        key={q.id}
                        className="p-3 rounded-xl border border-slate-200/80 bg-slate-50/70 space-y-2 text-xs"
                      >
                        <div className="flex items-center justify-between font-bold text-slate-800">
                          <span className="flex items-center gap-1">
                            <span className="w-5 h-5 rounded-md bg-teal-100 text-teal-800 text-[10px] flex items-center justify-center font-bold">
                              #{q.stepOrder}
                            </span>
                        ...
                        <div className="space-y-1.5">
                          {q.items?.map((it: any, sIdx: number) => (
                            <div
                              key={it.id}
                              className="p-2 rounded-lg bg-white border border-slate-200 text-slate-800 text-[11px] flex items-start gap-1.5"
                            >
                              <BookOpen className="w-3 h-3 text-teal-600 shrink-0 mt-0.5" />
                              <span className="font-medium line-clamp-2 leading-tight flex-1">
                                {it.material?.title || 'Materi Belum Terpilih'}
                              </span>
                            </div>
                          ))}
                        </div>
        ...
        {filteredRollings.length === 0 && (
          <div className="text-center py-14 px-4 bg-white/80 backdrop-blur-md rounded-2xl border border-dashed border-slate-200/90 flex flex-col items-center justify-center gap-2.5">
            <div className="w-12 h-12 rounded-2xl bg-teal-50 text-teal-700 flex items-center justify-center border border-teal-200/60 shadow-2xs">
              <Layers className="w-6 h-6" />
            </div>
            <h4 className="font-bold text-sm sm:text-base text-slate-800">
              Belum Ada Konfigurasi Rolling Materi
            </h4>
            <p className="text-xs text-slate-500 max-w-md text-center leading-relaxed">
              Buat konfigurasi antrean materi bergulir untuk memudahkan pengajar dan pengurus
              menjalankan kurikulum terjadwal per sesi, mingguan, atau bulanan.
            </p>
            {canManage && (
              <button
                type="button"
                onClick={handleOpenCreateModal}
                className="mt-2 inline-flex items-center gap-1.5 px-4 py-2 bg-teal-600 hover:bg-teal-500 text-white rounded-xl text-xs font-bold transition-all active:scale-95 shadow-2xs cursor-pointer"
              >
                <Plus className="w-4 h-4" />
                <span>Mulai Buat Rolling Materi</span>
              </button>
            )}
          </div>
        )}
