...
export type ClassRoleType = 'manage' | 'teacher' | 'parent' | 'student';

export interface ClassRoleTabItem {
  id: ClassRoleType;
  label: string;
  roleTitle: string;
  badge: string;
  subtitle: string;
  iconName: 'ShieldCheck' | 'GraduationCap' | 'Heart' | 'School';
  colorTheme: 'emerald' | 'teal' | 'indigo' | 'amber';
  href: string;
}
    ...
    <div className="w-full bg-white/90 backdrop-blur-md rounded-3xl border border-slate-200/80 p-3.5 sm:p-5 shadow-xs space-y-3 sm:space-y-4 animate-fade-in">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-500 hidden sm:block">
              Akun Anda memiliki beberapa wewenang kelas. Silakan pilih tampilan menu sesuai kebutuhan Anda saat ini.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
        {availableRoles.map((roleTab) => {
          ...
          const Icon = ICON_MAP[roleTab.iconName] || School;
