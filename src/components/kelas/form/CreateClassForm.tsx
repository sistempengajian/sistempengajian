...
import { ClassFormReferenceData } from '../types';
import { createClass } from '@/app/(protected)/kelas/actions';
...
export default function CreateClassForm({
  ...
  const [generationId, setGenerationId] = useState(
    referenceData.generations[0]?.id || ''
  );
  const [organizationId, setOrganizationId] = useState(
    referenceData.organizations[0]?.id || ''
  );
  ...
  // Auto-generate nama kelas rekomendasi jika nama masih kosong
  const handleAutoSuggestName = () => {
    if (selectedGeneration && selectedOrganization) {
      setName(`Kelas ${selectedGeneration.name} (${selectedOrganization.name})`);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.success) {
        setErrorMessage(res.message || 'Gagal menyimpan kelas.');
      ...
      setErrorMessage(err?.message || 'Terjadi kesalahan sistem saat membuat kelas.');
      setIsSubmitting(false);
    }
  };
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight">
            Tambah Kelas Pengajian Baru
          </h1>
          <p className="text-xs sm:text-sm text-slate-500 font-medium">
            Tentukan identitas kelas, jenjang usia, wilayah binaan, dan penugasan wali kelas
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="p-5 sm:p-6 rounded-3xl bg-white border border-slate-200/80 shadow-xs space-y-4">
          <div className="flex items-center gap-2.5 pb-3 border-b border-slate-100">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500">Nama kelas identik dan tahun ajaran aktif</p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                placeholder="Contoh: Kelas Caberawit Aisyah (Kelompok Klender)"
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-[11px] text-slate-400">
                Format standar: YYYY/YYYY (misal: 2026/2027)
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="p-5 sm:p-6 rounded-3xl bg-white border border-slate-200/80 shadow-xs space-y-4">
          ...
          <div className="space-y-4">
            ...
            {/* Pilihan Wilayah Binaan (Memanfaatkan Modal Pemilih Wilayah) */}
            <div className="space-y-1.5 pt-2">
              ...
              <OrganizationSelectTrigger
                ...
                allowClear={false}
                placeholder="Pilih Wilayah Binaan untuk Kelas Ini..."
              />
              ...
              <OrganizationSelectModal
                ...
                title="Pilih Wilayah Binaan Kelas"
